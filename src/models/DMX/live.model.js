...
class Live {
  constructor() {
    if (!Live.instance) {
      ...
      this.w = new Worker(new URL('./worker.js', import.meta.url), {
        type: 'module',
      });
    ...
    return Live.instance || this; // Updating singleton's parameters
  }
  ...
  add(updateFunction, quantize = 0) {
    if (quantize) {
      ...
      const animation = new Animation(((t) => {
        const tick = ((parseInt(t) / parseInt(this.beatDuration)) % quantize);
        if (tick >= quantize - 1 && tick <= quantize - 1 + QUANTIZATION_TOLERANCE) {
          const animationIndex = this.animations.findIndex((animation) => animation.id == animationId);
          if (animationIndex > -1) {
    ...
    const animation = new Animation(updateFunction, this.genAnimationId());
    this.animations.push(animation);
    return animation.id;
  }
