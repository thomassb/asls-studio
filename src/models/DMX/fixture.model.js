...
class Fixture extends Proxify {
   ...
   * @param {Object} data.manufacturer Fixture manufacturer name as described in public/fixture_list.json
   * @param {Object} data.model Manufacturer's fixture model name as described in public/fixture_list.json
  ...
  constructor(data = DEFAULT_FIXTURE_DATA) {
    ...
    this.universe = parseInt(data.universe);
    ...
    this.chStart = parseInt(data.chStart);
    this.channels = [];
    ...
    this.channels.forEach((channel) => {
      if (channel.fineChannelAliases && !channel.isFine) {
        channel.fineChannels = channel.fineChannelAliases.flatMap((alias) => {
          ...
          const fineChannel = this.channels.find((channel) => channel.type.replace(' ', '').toLowerCase() === standardizedAlias);
          return fineChannel || [];
        });
      }
    });
