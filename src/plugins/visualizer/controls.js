...
class Controls {
  constructor() {
    if (!Controls.instance) { // Ensuring singlelessness
      this.handle = null; // Initialising THREE.TransformControls handle
      this.mode = CONTROL_MODES.DISCRETE; // Setiting up control mode
      this.pooledInstances = []; // Initilising instance pool
      this.boundingBox = { // Preparing bounding box object
        min: new THREE.Vector3(),
        max: new THREE.Vector3(),
      };
      this.boundingBoxMesh = new THREE.Mesh(boundingBoxGeometry, boundingBoxMaterial); // Instanciating bounding box mesh
    ...
    return Controls.instance || this; // Updating singleton's parameters
  }
  ...
  applyTransformation() {
    for (let i = this.groupedInstances.children.length - 1; i >= 0; i--) {
      ...
      const instanceHandle = this.pooledInstances.find((x) => x._3DModel._dummy === child);
      if (instanceHandle) {
  ...
  showHelpers() {
    if (this.pooledInstances.length) {
      ...
      this.boundingBoxMesh.position.set(this.boundingBox.min.x + bbW / 2, this.boundingBox.min.y + bbH / 2, Math.max(this.boundingBox.min.z + bbD / 2, 0.51));
  ...
  detachAll() {
    ...
    // position.set(0,0,0)
    // position2.set(0,0,0)
    // this.controlHandle.target.copy(position)
    // this.cameraHandle.position.copy(position2)
  }
}

const controlsInstance = new Controls(); // Instanciating Controls
