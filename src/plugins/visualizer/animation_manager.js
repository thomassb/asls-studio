...
class AnimationManager {
  constructor() {
    if (!AnimationManager.instance) { // Ensuring singleness
      this.animations = []; // Preparing animations array
      this.update(); // Launchin update cycle
    }
    return AnimationManager.instance || this; // Updating singleton's parameters
  }
  ...
  /**
    ...
    */
  add(loop) {
    const animation = new Animation(loop); // Creating a new Animation instance from the provided looop
  ...
  /**
    ...
    */
  dispose(animation) {
    const { id } = animation; // Fetching the animation's id
    const index = AnimationManager._getAnimationIndex(id, this.animations); // Finding the animation's index within the instance's array
  ...
  static _getAnimationIndex(animationId, animations) {
    return (animations.findIndex((animation) => // Returning index
      animation.id === animationId, // Searching id matches
    ));
  }
}
...
const instance = new AnimationManager(); // Ensuring singleness
