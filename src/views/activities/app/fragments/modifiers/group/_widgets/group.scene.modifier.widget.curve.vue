...
export default {
  ...
  methods: {
    ...
    dragCp(e, controlPointId) {
      ...
      this.fade.controlPoints[controlPointId].y = parseFloat(Math.min(Math.max(1.0 - posY / this.canvas.height, 0), 1.0).toFixed(2));
    ...
    update() {
      ...
      this.context.lineTo(
        ...
        Math.min((1.0 - this.fade.controlPoints[0].y) * (this.canvas.height + 18), this.canvas.height - 6),
      );
      this.context.stroke();
